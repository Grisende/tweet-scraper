...
class Progress_Bar:
    ...
    def print_progress(self, current, waiting, retry_cnt, no_tweets_limit) -> None:
        ...
        if no_tweets_limit:
            if waiting:
                sys.stdout.write(
                    "\rTweets scraped : {} - esperando para acessar tweets antigos {} min 15 min".format(
                        current, retry_cnt
                    )
                )
            else:
            ...
            if waiting:
                sys.stdout.write(
                    "\rProgresso: [{:<40}] {:.2%} {} of {} - esperando para acessar tweets antigos {} min 15 min".format(
                        progress_bar, progress, current, self.total, retry_cnt
                    )
                )
            else:
                sys.stdout.write(
                    "\rProgresso: [{:<40}] {:.2%} {} of {}                                                  ".format(
